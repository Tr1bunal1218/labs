...
def jordan_gauss_method(matrix):
    ...
    for c in range(len(matrix)):
        ...
        for i in range(0, len(matrix)):
            ...
            if i == c:
                continue
            k = matrix[i][c] * Fraction(-1)
            for j in range(c, len(matrix[0])):
                # По элементам строк
                matrix[i][j] = matrix[i][j] + matrix[c][j] * k
        show_mtx(matrix)

    # Проверка - строка не нулевая (нет решений)
    counter_no_null_str = 0
    for i in matrix:
        flag = True
        for j in i[:-1]:
            if j != Fraction(0):
        ...
        if flag and i[-1] != Fraction(0):
            ...
            continue
        counter_no_null_str += 1

    print("Answer:")
    # Нет корней
    if not counter_no_null_str:
        print("Not solves")
    # Все корни известны (одно решение)
    elif counter_no_null_str == len(matrix) and counter_no_null_str == len(matrix[0]) - 1:
        tmp = list()
...
for line in f:
    ...
    mtx.append(list(map(Fraction, a)))
jordan_gauss_method(mtx)
